  ...
  return (
    <>
      ...
      <article className="quote-card">
        <div className="quote-card-head"><div><h3>Request for Quotations</h3><span className="quote-chip">Live</span><span className="quote-chip">Archived</span></div><div className="quote-actions"><Btn icon={Filter}>Filter</Btn><Btn icon={Download}>Export</Btn></div></div>
        <Table columns={[
...
function GenericList({ kind, nav }) {
  ...
  return <><article className="quote-card"><div className="quote-card-head"><h3>{kind === 'revisions' ? 'Tracking Log' : kind === 'expired' ? 'Expired Quote Registry' : 'Rejected Quote Registry'}</h3><div className="quote-actions"><Btn icon={Filter}>Filters</Btn><Btn icon={Download}>Export</Btn></div></div><Table columns={[
    { key: 'id', label: 'Quote' },
...
function Reports() {
  ...
  return <><div className="quote-filterbar"><Calendar size={18} /><select><option>Last 30 Days</option></select><select><option>All Statuses</option></select><Btn icon={Download}>Export PDF</Btn><Btn icon={Download}>Export Excel</Btn><Btn icon={Download}>Export CSV</Btn></div><section className="quote-kpi-grid">{reports.map((r) => <article className="quote-card" key={r}><FileText /><h3>{r}</h3><p>Ready for executive and procurement review.</p><div className="quote-actions"><Btn>PDF</Btn><Btn>Excel</Btn><Btn>CSV</Btn></div></article>)}</section><article className="quote-card"><h3>Recent Exports</h3><Table columns={[{ key: 'id', label: 'Report' }, { key: 'seller', label: 'Owner' }, { key: 'submitted', label: 'Date' }, { key: 'status', label: 'Status', render: () => <Badge status="Approved">Ready</Badge> }]} rows={quotations.slice(0, 4)} /></article></>;
}
